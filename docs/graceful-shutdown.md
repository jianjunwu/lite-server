...
lite-server drains in-flight work on `SIGTERM`/`SIGINT` (P4-2) so a rolling
update does not cut requests mid-flight. This doc is the operator-facing
companion to that behavior — the rolling-update 502 is the single most common
gateway/backend failure, and it is almost always a misconfigured drain window.

## What the server does on SIGTERM

1. **Health fails fast (LB摘流).** The shared `draining` flag flips on at the
   very start of shutdown:
   ...
   A Kubernetes readiness/liveness probe and a gRPC LB health watcher therefore
   see the server as not-ready **before** connections are torn down.
2. **Stop accepting new connections / streams.** HTTP stops `accept()`; gRPC
   sends `GOAWAY` and rejects new streams (tonic `serve_with_shutdown` /
   `serve_with_incoming_shutdown`).
3. **Drain in-flight.** In-flight HTTP requests and gRPC RPCs are allowed to
   finish. A long-lived SSE / gRPC stream runs until it ends naturally or the
   grace window expires.
4. **Grace backstop.** After `server.graceful_timeout` (default `30`s), still-
   running server tasks are force-aborted. (An individual request is also bound
   by its own per-request deadline, `server.timeout`, and the worker unload
   grace — so a stuck request never pins the process open forever.)
...
- `slack` (≈10–15s): worker unload, OS teardown, slow shutdown of long streams.
